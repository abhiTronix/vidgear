...
def test_screengear(monitor, options, colorspace):
	...
	try:
		...
		stream = ScreenGear(monitor=monitor, logging=True, colorspace = colorspace, **options).start() 
		#playback
		i = 0
		while (i<20):
			frame = stream.read()
		...
		if platform.system() == 'Linux' or platform.system() == 'Windows' or monitor in [-1, 3]:
			logger.exception(e)
		else:
			pytest.fail(str(e))