...
def mkdir_safe(dir, logging = False):
	...
	try:
		os.makedirs(dir)
		if logging: logger.debug("Created directory at `{}`".format(dir))
	except OSError as e:
		if e.errno != errno.EEXIST: raise
		if logging: logger.debug("Directory already exists at `{}`".format(dir))
...
async def generate_webdata(path, overwrite_default = False, logging = False):
	...
	if overwrite_default:
		...
		await download_webdata(template_dir, files = ['index.html', '404.html', '500.html', 'base.html'], logging = logging)
		await download_webdata(css_static_dir, files = ['bootstrap.min.css', 'cover.css'], logging = logging)
		await download_webdata(js_static_dir, files = ['bootstrap.min.js', 'jquery-3.4.1.slim.min.js', 'popper.min.js'], logging = logging)
		await download_webdata(favicon_dir, files = ['favicon-32x32.png'], logging = logging)
	else:
...
async def download_webdata(path, files = [], logging = False):
	...
	async with aiohttp.ClientSession(raise_for_status=True, read_timeout=4) as session:
		tasks = [download(session, file_path, url, logging = logging) for (url, file_path) in targets]
		await asyncio.gather(*tasks)
	
	#verify downloaded data
	if logging: logger.debug("Verifying downloaded data at `{}`:".format(path))
...
async def download(session, target, url, logging = False):
	...
	async with session.get(url) as response:
		...
		if logging: logger.debug("Downloading  data-file: {}.".format(target))
		total_length = int(response.headers.get('content-length', 0)) or None
		assert not(total_length is None and total_length > 0), "[Helper:ERROR] :: Failed to retrieve files, check your Internet connectivity!"
		...
		with open(target, mode='wb') as f:
			async for chunk in response.content.iter_chunked(512):
				f.write(chunk)
				if len(chunk) > 0: bar.update(len(chunk))
		bar.finish()
