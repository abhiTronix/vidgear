...
> Threaded Queue Mode is designed exclusively for VidGear's Videocapture Gears (namely CamGear, VideoGear) and few Network Gears (such as NetGear at the client's end) for achieving high-performance, asynchronous, error-free video-frames handling. 
...
!!! info "Threaded-Queue-Mode is **NOT** required and thereby automatically disabled for live feeds such as camera devices/modules."

&nbsp; 

## What does Threaded-Queue-Mode exactly do?

Threaded-Queue-Mode helps VidGear perform the threaded video-processing tasks in a highly optimized, well-organized, and most competent way possible: 

### A. Enables Multi-Threading

> In case you don't already know, OpenCV's `read()` is a [**Blocking I/O**](https://luminousmen.com/post/asynchronous-programming-blocking-and-non-blocking) function for reading and decoding the next video-frame, and consumes much of the I/O-bound memory depending upon our video source properties and system hardware. This essentially means, the corresponding thread that reads data from it is continuously blocked from retrieving the next frame. As a result, our Python program appears slow and sluggish even without any type of computationally expensive image processing operations. This problem is far more severe on low-memory SBCs like Raspberry Pis.

In Threaded-Queue-Mode, VidGear creates several [**Python Threads**](https://docs.python.org/3/library/threading.html) within one process to offload the frame-decoding task to a different thread. Thereby, VidGear is able to execute different video I/O-bound operations at the same time by overlapping their waiting times. Moreover, threads are managed by the operating system itself and are capable of distributing them between available CPU cores efficiently. In this way, Threaded-Queue-Mode keeps on processing frames faster in the [background](https://en.wikipedia.org/wiki/Daemon_(computing)) without affecting our main Python program thread.
...
These problems are avoided in Threaded-Queue-Mode by utilizing **Thread-Safe, Memory-Efficient, and Fixed-Size `Queues`** (_with approximately same O(1) performance in both directions_), that isolate the frame-decoding thread from other parallel threads and provide synchronized access to incoming frames without any obstruction. 

### C. Accelerates Frame Processing

With queues, VidGear always maintains a fixed-length frames buffer in memory and blocks the thread temporarily if the queue is full to avoid possible frame drops or otherwise pops out the frames synchronously without any obstruction. This significantly accelerates frame processing rate (and therefore our overall video processing pipeline) by dramatically reducing latency — since we don’t have to wait for the `read()` method to finish reading and decoding a frame; instead, there is always a pre-decoded frame ready for us to process.
...
- [x] _Utilizes thread-safe, memory-efficient `queues` that append and pop frames with the same O(1) performance from either side._
...
!!! warning "Important Warnings"
        ...
        * Disabling Threaded-Queue-Mode does **NOT** disable multi-threading.

        * `THREADED_QUEUE_MODE` attribute does **NOT** work with live feeds, such as camera devices/modules.
...
and you can pass it to the `options` dictionary parameter of the respective API.
